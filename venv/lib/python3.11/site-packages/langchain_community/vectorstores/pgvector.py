...
class PGVector(VectorStore):
    ...
    def delete_collection(self) -> None:
        ...
        with Session(self._bind) as session:
            ...
            if not collection:
                self.logger.warning("delete_collection: Collection not found")
                return
        ...
        with Session(self._bind) as session:
            ...
            for text, metadata, embedding, id in zip(texts, metadatas, embeddings, ids):
                embedding_store = self.EmbeddingStore(
                    embedding=embedding,
                    document=text.replace('\x00', ''),
                    cmetadata=metadata,
                    custom_id=id,
                    collection_id=collection.uuid,
                )
                documents.append(embedding_store)
            session.bulk_save_objects(documents, update_changed_only=True)
            session.commit()
    ...
    def similarity_search(
        ...
        **kwargs: Any,
        ...
        """
        # print(f"pgvector similarity search: {traceback.print_stack()}")
    ...
    def similarity_search_with_score_by_vector(
        ...
        # for result in results:
            # print(f"similarity search with score by vector result: {result.__str__()}")
            # print(f"similarity search with score by vector result embedding: {result.EmbeddingStore.embedding}")
            # print(f"similarity search with score by vector result document: {result.EmbeddingStore.document}")
    ...
    async def from_texts(
        ...
        **kwargs: Any,
        ...
        """
        embeddings = await embedding.aembed_documents(texts=list(texts), chunk_size=chunk_size) ## calls open ai embed_documents?
